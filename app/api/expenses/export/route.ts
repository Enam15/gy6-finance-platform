...
/**
 * Expense list export (CSV or XLSX). Mirror of /api/income/export with
 * Payee account in place of Client account.
 */
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (format === "xlsx") {
    const rows: XlsxRow[] = entries.map((e) => ({
      ...
      account: accountName.get(e.payeeAccountId) ?? "Unknown",
    ...
    const columns: XlsxColumn<XlsxRow>[] = [
      ...
      { key: "account", label: "Payee account", width: 25 },
    ...
    const buffer = await toXlsx("Expenses", columns, rows);
    return new Response(new Uint8Array(buffer), {
      headers: {
        "content-type": XLSX_MIME,
        "content-disposition": `attachment; filename="gy6-expenses-${stamp}.xlsx"`,
      },
    });
  }
  ...
  const columns: CsvColumn<CsvRow>[] = [
    ...
    { key: "account", label: "Payee account" },
  ...
  return new Response(csv, {
    headers: {
      ...
      "content-disposition": `attachment; filename="gy6-expenses-${stamp}.csv"`,
    },
  });
}
