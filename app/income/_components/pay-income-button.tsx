...
interface PayIncomeButtonProps {
  entryId: string;
  description: string;
...
/**
 * Inline "Pay" action on a confirmed income row. Opens a dialog with the
 * Business-account picker, an amount input pre-filled to the outstanding
 * total, paid-on date, and an optional note. Submits to
 * POST /api/income/[id]/payments and refreshes the page.
 */
export function PayIncomeButton({
  entryId,
  description,
  amountDueMinor,
  businessAccounts,
}: PayIncomeButtonProps) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/income/${entryId}/payments`, {
        method: "POST",
      ...
      <DialogContent className="sm:max-w-lg">
        <form onSubmit={onSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              {description} - outstanding {amountDueDisplay}. Posts DR
              business account, CR client account.
            </DialogDescription>
          </DialogHeader>

          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="pay-income-account">Business account</Label>
              <Select
                value={businessAccountId}
                onValueChange={(v) => setBusinessAccountId(v ?? "")}
              >
                <SelectTrigger id="pay-income-account">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-2">
                <Label htmlFor="pay-income-amount">Amount</Label>
                <Input
                  id="pay-income-amount"
                  value={amount}
              ...
              <div className="grid gap-2">
                <Label htmlFor="pay-income-date">Paid on</Label>
                <Input
                  id="pay-income-date"
                  type="date"
                  value={paidOn}
            ...
            <div className="grid gap-2">
              <Label htmlFor="pay-income-note">Note (optional)</Label>
              <Input
                id="pay-income-note"
                value={note}
